...
python annotation.py get_stanford_tile_classification_annotation ${ANNOTATION_DIR} ${RESIZED_LONG_EDGE} ${RESIZED_SHORT_EDGE} ${TILE_WIDTH} ${TILE_HEIGHT} ${TILE_ANNOTATION_DIR} | tee ${TILE_ANNOTATION_DIR}/stats.txt

echo "Sample Tile Images for Training to ${EXPERIMENT_DIR}"
mkdir -p ${EXPERIMENT_DIR}
ln -s ${RESIZED_IMAGE_DIR} ${EXPERIMENT_DIR}/photos
python preprocess.py sample_stanford_frames ${TILE_ANNOTATION_DIR} ${SAMPLE_NUM_PER_CLASS} ${EXPERIMENT_DIR} train &
sleep 10
python preprocess.py sample_stanford_frames ${TILE_ANNOTATION_DIR} ${SAMPLE_NUM_PER_CLASS} ${EXPERIMENT_DIR} test &
wait
