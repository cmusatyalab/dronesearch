r"""Finetune Inception v3 or Mobilenet models.
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        # jj: sort the file names so that we can do validation and testing by video
        file_list.sort()
...
def construct_graph(model_info, class_count, bottleneck_tensor, final_tensor_name):
    ...
    with tf.name_scope('input'):
        jpeg_data_input, decoded_image_tensor = add_jpeg_decoding(
    ...
    with tf.name_scope(layer_name):
        with tf.name_scope('weights'):
            initial_value = tf.truncated_normal(
              [model_info['bottleneck_tensor_size'], class_count],
              stddev=0.001)
    ...
    return (train_step, cross_entropy_mean, jpeg_data_input,
            decoded_image_tensor, ground_truth_input,
            final_tensor)
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        (train_step, cross_entropy, jpeg_data_input,
         decoded_image_tensor, ground_truth_input,
         final_tensor) = construct_graph(model_info, len(image_lists.keys()),
                                         bottleneck_tensor,
                                         FLAGS.final_tensor_name)

        # Create the operations we need to
        # evaluate the accuracy of our new layer.
        ...
        for i in range(FLAGS.how_many_training_steps):
            # Retrieve batch_size of file names
