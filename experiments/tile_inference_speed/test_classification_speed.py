...
def read_tensor_from_image_file(input_height=299,
    ...
    resized = tf.image.resize_bilinear(float_caster,
...
def _divide_to_tiles(self, im, grid_w, grid_h):
    ...
    for h_idx in range(0, grid_w):
        for v_idx in range(0, grid_h):
            ...
            tile_w = int(min(tile_w, im_w - tile_x))
            tile_h = int(min(tile_h, im_h - tile_y))
...
if __name__ == "__main__":
    ...
    with tf.Session(graph=graph) as sess:
        # warm up
        image_np = (np.random.rand(1, 224, 224, 3) * 255).astype(np.uint8)
        ...
        for _ in range(3):
            for image_path in test_images_path:
                ...
                tiles = _divide_to_tiles(image_np)
                # Actual detection.
                st = time.time()
                normalized_image = sess.run(normalized, {
                    input_image: tiles
